...
//    private static Object User;
    public static InputCommand parseInput(String inputString) {
        ...
        } else if(commandIdentifier.equals("EDIT_EXPENSE") && inputCommandParts.length > 5) {

            Integer expenseId = Integer.parseInt(inputCommandParts[1]);
            String description = inputCommandParts[2];
            Double cost = Double.parseDouble(inputCommandParts[3]);
            Integer payerId = Integer.parseInt(inputCommandParts[inputCommandParts.length-1]);
            ArrayList<Integer> debterId = new ArrayList<>();
            for(Integer i=4; i<inputCommandParts.length-1; i++) {
                debterId.add(Integer.parseInt(inputCommandParts[i]));
            }

            return new EditExpenseInput(expenseId, description, cost, debterId, payerId);
        }
